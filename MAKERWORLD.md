...
⚠️ **Ссылка на сайт в Description есть, но домен ещё не отвечает.** Сайт
задеплоен и работает на `worldbytouch.pages.dev` (200, обе языковые версии),
а у `worldbytouch.com` нет A-записи — кастомный домен не привязан к проекту
Pages. Привязка делается в Cloudflare Pages → Custom domains, записи создадутся
сами, NS домена уже на Cloudflare. **Публиковать модель только после того, как
`worldbytouch.com` начнёт отвечать** — иначе первый же посетитель уткнётся в
мёртвую ссылку. Проверка: `curl -sI https://worldbytouch.com`.
